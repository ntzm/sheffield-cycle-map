...
function formatOpeningHours(raw) {
  ...
  try {
    const todayIdx = (new Date().getDay() + 6) % 7; // 0 = Monday, ..., 6 = Sunday
    ...
    if (isFullWeek) {
      return [{ label: "Open 24/7", value: "", isToday: true }];
    }

    // Slice day-by-day so 24/7 spans split correctly.
    const dayLines = DAY_NAMES.map((label, idx) => {
      ...
      return { label, value: text, isToday: idx === todayIdx };
    });
    ...
    for (let i = 1; i <= dayLines.length; i++) {
      ...
      if (isBreak) {
        ...
        const isTodayInRange = todayIdx >= start && todayIdx <= end;
        grouped.push({
          label,
          value: prev.value,
