export class LayerControl {
  ...
  onAdd(map) {
    ...
    this._layers.forEach(layerConfig => {
      ...
      item.className = 'maplibre-layer-control__item' + (parentId ? ' maplibre-layer-control__item--child' : '');
      ...
      checkbox.addEventListener('change', () => {
        ...
        targets.forEach((targetLayerId) => {
          if (this._map.getLayer(targetLayerId)) {
            this._map.setLayoutProperty(targetLayerId, 'visibility', visibility);
          }
  ...
  _updateParentState(parentId) {
    ...
    childIds.forEach(cid => {
      ...
      if (!cb) return;
      if (cb.checked) visibleCount += 1; else hiddenCount += 1;
    });
