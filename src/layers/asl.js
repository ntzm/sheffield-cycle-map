...
export async function addAslLayer(map, urlState, cacheBust) {
  const iconId = 'asl-icon';
  if (!map.hasImage(iconId)) {
    const svg = await fetch(`icons/asl.svg?v=${cacheBust}`).then(r => r.text());
    await addSvgImage(map, iconId, svg, { pixelRatio: 2 });
  }

  map.addSource('asl', {
    type: 'geojson',
    data: `data/asl.geojson?v=${cacheBust}`
  });
  ...
  map.addLayer({
    ...
    source: 'asl',
    layout: {
      visibility: initialVisible(urlState, 'asl-layer', false) ? 'visible' : 'none',
      'icon-image': iconId,
      'icon-size': [
