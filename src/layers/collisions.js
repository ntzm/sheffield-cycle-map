...
const LIGHT_LABELS = {
  '1': 'Daylight', '4': 'Dark, street lights lit', '5': 'Dark, street lights unlit',
  '6': 'Dark, no street lights', '7': 'Dark, street lights unknown'
};
const ROAD_LABELS = {
...
export async function addCollisions(map, urlState) {
  ...
  map.on('click', 'dft-collisions-layer', (e) => {
    ...
    const { root, heading } = createPopupContainer(`Collision on ${p.date} at ${p.time}`);
    ...
    modeChip.textContent = p.urban_or_rural === '1' ? 'Urban' : (p.urban_or_rural === '2' ? 'Rural' : 'Unknown area');
