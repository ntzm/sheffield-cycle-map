...
export async function addPumpsLayer(map, urlState, cacheBust) {
  ...
  await Promise.all([
    fetch(PUMP_ICON).then(r => r.text()).then(svg => addSvgImage(map, 'pump-icon', svg, { pixelRatio: 2 })),
  ...
  map.on('click', 'pumps-layer', (e) => {
    ...
    const label = isBroken(props) ? 'Vandalised public bike pump' : 'Public bike pump';
  ...
  map.on('mouseenter', 'pumps-layer', () => { map.getCanvas().style.cursor = 'pointer'; });
  map.on('mouseleave', 'pumps-layer', () => { map.getCanvas().style.cursor = ''; });
}
