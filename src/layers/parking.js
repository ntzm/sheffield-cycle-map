...
function buildParkingPopup(feature) {
  ...
  if (props.fee) chipData.push({ text: props.charge ? `Fee: ${props.charge}` : 'Fee', tone: 'warn' });
  if (props.access) {
    const accessTone = /(private|customer|customers|member|students?|employees?)/i.test(props.access) ? 'warn' : 'neutral';
    chipData.push({ text: props.access, tone: accessTone });
  }
  if (props.wheel_benders === true || props.wheel_benders === 'true') chipData.push({ text: 'Wheelbenders', tone: 'alert' });
  if (props.capacity) {
  ...
  if (props.covered === 'yes' || props.covered === 'true' || props.covered === 'Yes') {
  ...
  } else if (props.covered === 'no' || props.covered === 'false' || props.covered === 'No') {
  ...
  const osmTypeChar = osmType === 'way' ? 'way' : osmType === 'relation' ? 'relation' : 'node';
  if (props.imageHref) {
...
export function attachParkingInteractions(map, layerId) {
  map.on('click', layerId, (e) => {
    ...
    const popup = new maplibregl.Popup().setLngLat(coords).setDOMContent(buildParkingPopup(feature));
