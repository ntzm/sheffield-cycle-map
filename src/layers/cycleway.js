...
export function addCycleway(map, urlState, cacheBust) {
  ...
  const onewayFactor = ['case', ['==', ['get', 'effectiveOneway'], 'yes'], 0.5, 1];
  const trackSideSign = ['match', ['get', 'trackSide'], 'left', -1, 'right', 1, 0];
  const laneSideSign = ['match', ['get', 'laneSide'], 'left', -1, 'right', 1, 0];
  ...
  const pathPaint = {
    ...
    'line-offset': [
      'interpolate', ['linear'], ['zoom'],
      10, ['*', trackSideSign, 1.2],
      13, ['*', trackSideSign, 2.1],
      16, ['*', trackSideSign, 3.6],
      18, ['*', trackSideSign, 5.2]
    ]
  };
  ...
  map.addLayer({
    ...
    layout: {
      'line-join': 'round',
      'line-cap': 'round',
      visibility: initialVisible(urlState, 'cycleway-lane-wide-layer', true) || urlState.visibleLayers.has('cycleway-layer') ? 'visible' : 'none'
    },
    paint: {
      ...laneBasePaint,
      'line-color': '#c63b2b',
  ...
  map.addLayer({
    ...
    layout: {
      'line-join': 'round',
      'line-cap': 'round',
      visibility: initialVisible(urlState, 'cycleway-lane-narrow-layer', true) || urlState.visibleLayers.has('cycleway-layer') ? 'visible' : 'none'
    },
    paint: {
      ...laneBasePaint,
      'line-color': '#e58f85',
