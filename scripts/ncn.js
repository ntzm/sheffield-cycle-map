...
import { runOverpass, writeGeojson, SHEFFIELD_AREA_ID, asLineString, DATA_DIR } from './lib/overpass.js';
...
async function main() {
  ...
  const boundary = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'boundary.geojson'), 'utf8')).features[0].geometry;

  const features = (data.elements || [])
    .map(e => asLineString(e, e.tags?.ref ? { ref: e.tags.ref } : {}))
    .filter(Boolean)
    .flatMap(f => clipLineToPolygon(f, boundary))
    .filter(f => f.geometry.coordinates.length > 1);

  writeGeojson('ncn.geojson', features);
}
...
function segmentIntersect(p0, p1, q0, q1) {
  const x1 = p0[0], y1 = p0[1], x2 = p1[0], y2 = p1[1];
  const x3 = q0[0], y3 = q0[1], x4 = q1[0], y4 = q1[1];
