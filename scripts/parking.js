...
import { cachedJsonFetch, cachedFetch, getCacheEntry, setCacheEntry } from "./lib/cache.js";
...
function panoramaxIdsFromTags(tags) {
	return Object.entries(tags)
		.filter(([key, value]) => value && (key === "panoramax" || key.startsWith("panoramax:")))
		.map(([_, value]) => value);
}
...
async function getPanoramaxData(id) {
	const response = await cachedJsonFetch(`https://panoramax.mapcomplete.org/api/search?limit=1&ids=${id}`, {
		headers: {
	...
	return { thumbnailHref, license, producer, width, height, qualityScore, blurhash };
}

async function getBlurhash(imageBuffer) {
	try {
		const image = sharp(imageBuffer).resize({ width: 64, height: 64, fit: "inside" }).ensureAlpha();
...
async function getQualityScore(thumbnailHref, imageBuffer) {
	...
	try {
		...
		console.warn(`Failed to score image ${thumbnailHref}:`, err?.message ?? err);
		return undefined;
	}
}
...
async function runBrisque(base64) {
	return new Promise((resolve, reject) => {
		...
		proc.on("close", (code) => {
			if (code !== 0) {
				return reject(new Error(`BRISQUE script failed (${code}): ${stderr.trim()}`));
			}
			try {
