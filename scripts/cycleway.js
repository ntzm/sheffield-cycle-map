...
const isLaneValue = (value) => typeof value === 'string' && value.includes('lane');

const matchSideOrDefault = (tags) => {
  const sideTag = tags['cycleway:side'];
  if (sideTag === 'left' || sideTag === 'right') return sideTag;
...
const isNo = (value) => typeof value === 'string' && ['no', '0', 'false'].includes(value.toLowerCase());
...
async function main() {
  ...
  for (const e of data.elements || []) {
    ...
    const underConstruction = tags.highway === 'construction' || tags.construction !== undefined;
    ...
    const effectiveOneway = computeEffectiveOneway(tags, hasTrack, baseProps.oneway);
    ...
    // A plain cycleway=lane should be interpreted as lanes on both sides, the
    // same as cycleway:both=lane. Otherwise we default to the tagged side
    // (or UK-left when unspecified).
    const isPlainGenericLane = isLaneValue(tags.cycleway) &&
      !isLaneValue(tags['cycleway:left']) &&
      !isLaneValue(tags['cycleway:right']) &&
      !isLaneValue(tags['cycleway:both']);
    const plainLaneBothSides = isPlainGenericLane && effectiveOneway !== 'yes';
    const plainLaneLeftOnly = isPlainGenericLane && effectiveOneway === 'yes';
    ...
    if (isLaneWay) {
      ...
      if (leftWidth !== null) { addLane('left', leftWidth); anyWidthAdded = true; }
      if (rightWidth !== null) { addLane('right', rightWidth); anyWidthAdded = true; }
      ...
      if (genericWidth !== null) {
        if (isLaneValue(tags['cycleway:both']) || plainLaneBothSides) {
          ...
          addLane('left', genericWidth);
        } else if (isLaneValue(tags['cycleway:left']) && isLaneValue(tags['cycleway:right'])) {
          addLane('left', genericWidth);
          addLane('right', genericWidth);
        } else {
