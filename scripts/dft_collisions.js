...
const boundary = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'boundary.geojson'), 'utf8'));
...
async function processYear(year) {
  ...
  const fatalCollisionIds = new Set(
    accidents
      .filter((a) => String(a.collision_severity) === '1' || String(a.enhanced_severity_collision) === '1')
  ...
  for (const a of accidents) {
    ...
    if (cyclistCasualties === 0 && pedalCycleCollisionIds.has(a.collision_index) && fatalCollisionIds.has(a.collision_index)) {
    ...
    const props = {
      ...
      casualties: casualtyCounts[a.collision_index] || Number(a.number_of_casualties) || 0,
      ...
      other_vehicle_types: formatOtherVehicles(otherVehiclesByCollision[a.collision_index]),
    };
