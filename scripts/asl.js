...
function nodeBearing(node, roadsByNode) {
  ...
  for (const road of roads) {
    ...
    if (idx > 0 && geom[idx - 1]) {
      ...
      candidates.push({ dx: dxPrev, dy: dyPrev, len2: dxPrev * dxPrev + dyPrev * dyPrev, type: 'in' });
    }
    if (idx < geom.length - 1 && geom[idx + 1]) {
      const dxNext = (geom[idx + 1].lon - here.lon) * cosLat;
      const dyNext = geom[idx + 1].lat - here.lat;
      candidates.push({ dx: dxNext, dy: dyNext, len2: dxNext * dxNext + dyNext * dyNext, type: 'out' });
    }
    if (!candidates.length) continue;

    // Pick inbound if present, otherwise longest.
    let chosen = candidates.find(c => c.type === 'in') || candidates.reduce((a, b) => (b.len2 > a.len2 ? b : a));
    if (chosen.len2 <= 0) continue;

    const priority = MOTORISED_HIGHWAYS.has(road.tags?.highway) ? 2 : 1;

    if (!best || priority > best.priority || (priority === best.priority && chosen.len2 > best.len2)) {
