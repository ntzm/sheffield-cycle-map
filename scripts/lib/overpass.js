...
export const DATA_DIR = path.join(import.meta.dirname, '..', '..', 'public', 'data');

export async function runOverpass(query, { retries = 3, backoffMs = 1500 } = {}) {
  let attempt = 0;
  let lastErr;
  ...
  while (attempt <= retries) {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Overpass error ${res.status} ${res.statusText} ${text.slice(0, 200)}`);
      }
...
export function asLineString(e, props = {}) {
  if (e.type !== 'way' || !Array.isArray(e.geometry) || e.geometry.length < 2) return null;
  return {
    type: 'Feature',
