...
function makeKey(url, options = {}) {
  ...
  const payload = { url, method: method.toUpperCase(), body, headers: headersLower };
...
function encodeEntry(data, cacheType) {
  if (cacheType === 'buffer') {
    return { __cacheType: 'buffer', data: Buffer.from(data).toString('base64') };
  }
...
export async function cachedFetch(url, options = {}) {
  ...
  const stored = encodeEntry(data, responseType === 'arrayBuffer' ? 'buffer' : responseType === 'text' ? 'text' : 'json');
  cache[key] = stored;
  persistCache();
