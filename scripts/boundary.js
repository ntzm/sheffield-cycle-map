...
function stitchRings(ways) {
  const rings = [];
  const unused = new Map(ways.map(w => [w.id, w.geometry.map(p => [p.lon, p.lat])]));
...
async function main() {
  ...
  if (!relations.length) throw new Error('Relation not found in Overpass response');
  const relation = relations.find(r => r.tags?.admin_level === '8') || relations[0];
  ...
  writeGeojson('boundary.geojson', [{ type: 'Feature', properties: {}, geometry: geom }]);
}
