...
function simplifyRing(ring, maxPoints = 30) {
  ...
  if (out[out.length - 1][0] !== ring[ring.length - 1][0] ||
...
async function main() {
  ...
  console.log(`Fetching bicycle thefts for ${months.length} months (${months[0]} … ${months[months.length - 1]})`);
  ...
  for (const [idx, month] of months.entries()) {
    ...
    try {
      ...
      console.log(`Month ${month}: ${rows.length} records (total ${features.length})`);
    } catch (err) {
